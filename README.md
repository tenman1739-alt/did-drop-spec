...
4. Optional: The controller registers a Human-Readable Alias (e.g., warehouse@alias) and maps it to did:drop:mainnet:555123.
...
Since `did:drop` is often paired with a Human-Readable Alias Layer, an observer could correlate an alias with a specific landing pad location. Users should be aware that the link between their alias and their `did:drop` identifier is publicly visible on-chain.
